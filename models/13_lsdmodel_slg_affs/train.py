...
#This model has short and long range affinities, MSE loss, and no z intensity augmentation, and grow boundaries step 2
...
def train_until(
    ...
    source = tuple(
        ...
        + gp.Pad(mask, padding)
        + gp.RandomLocation()
        + gp.Reject(mask=mask, min_masked=0.01)
    ...
    pipeline += gp.GrowBoundary(
        labels, mask, steps=2, only_xy=True
    )
    ...
    pipeline += gp.Squeeze([raw,gt_affs,pred_affs,affs_scale, gt_lsds, pred_lsds, lsds_scale])
