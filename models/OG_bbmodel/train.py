...
def train_until(
    ...
    voxel_size = gp.Coordinate(zarr.open(f'{training_volumes[0]}/image_norm','r').attrs['voxel_size'])
    ...
    source = tuple(
        (
            gp.ArraySource(raw, open_ds(os.path.join(training_volumes[0],"image_norm")),True),
            gp.ArraySource(labels, open_ds(os.path.join(training_volumes[0],"label")),False)
        ) + gp.MergeProvider()
        + gp.Normalize(raw)
