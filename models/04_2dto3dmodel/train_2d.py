...
def train_until(
    ...
    source = tuple(
        ...
        + gp.Pad(mask, padding)
        + gp.RandomLocation()#mask=mask, min_masked=0.001)
        + gp.Reject(mask=mask, min_masked=0.001)
        for sample in training_volumes
    )

    pipeline = source + gp.RandomProvider()#probabilities=[0.05405405, 0.38378378, 0.13783784, 0.03783784, 0.38648649])
